...
module.exports = async(bot, msg) => {
    ...
    if(dat == 'roole' || dat == 'updateorder'){
        bot.editMessageText('🚩 Xozir qayerdasiz qaysi?',{
            chat_id: chatId,
            message_id: msgId,
    ...
    //     });
    // }
    else if(dat == 'time' || msg.data == 'to_cit=cancel'){  //to_city
        // if(msg.data != 'to_cit=cancel') await updateOrder(chatId, {time: data})
        if(msg.data != 'tim=cancel') await updateOrder(chatId, {date: data})
        bot.editMessageText('🏁 Boradigan viloyatingizni belgilang!',{
            chat_id: chatId,
            message_id: msgId,
...
async function button (kluch,data) {
    ...
    if(kluch) {
        city.map( cit => {
            array.push([{text:'🚩'+ cit.city_name, callback_data: 'date='+cit.city_id}]); //district
        });
...
async function button2 (kluch,data) {
    ...
    if(kluch) {
        ...
        array.push([{text: '🔙 Ortga', callback_data: 'date=cancel'}]);
    } else {
        city.map( cit => {
